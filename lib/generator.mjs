...
要求：面向${options.grade || "小学"}，共生成${options.count || 10}道不重复、无歧义、不过度刁钻的题；quizItems 优先使用单选和判断；pairs 生成 8 对；cards 生成 16 个适合朗读或口答的知识卡。答案必须准确。若材料不足，使用常识范围但不要虚构出处。`;
  const requestBody = { model, temperature: 0.4, messages: [{ role: "system", content: system }, { role: "user", content: prompt }] };
  ...
  return { pack: normalizePack(JSON.parse(jsonText), prompt), mode: "ai", notice: "题目已生成，请务必检查答案。" };
}
